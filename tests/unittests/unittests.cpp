...
namespace
{
    struct tracking_allocator
    {
        ...
        static inline std::size_t current_usage()
        {
            return usage.load(std::memory_order_relaxed);
        }
...
struct corealgos_allocator
{
    static inline void *malloc(std::size_t size)
    {
        return tracking_allocator::malloc(size);
    }
    static inline void free(void *ptr)
    {
        tracking_allocator::free(ptr);
    }
};
...
namespace moodycamel
{
    ...
    struct TestTraits : public MallocTrackingTraits
    {
        ...
        static inline std::atomic<int> &_malloc_count()
        {
            static std::atomic<int> c;
            return c;
        }
        static inline int malloc_count()
        {
            return _malloc_count().load(std::memory_order_seq_cst);
        }
        static inline std::atomic<int> &_free_count()
        {
            static std::atomic<int> c;
            return c;
        }
        static inline int free_count()
        {
            return _free_count().load(std::memory_order_seq_cst);
        }

        static inline void *malloc(ConcurrentQueueDefaultTraits::size_t bytes)
        {
            ++_malloc_count();
            return tracking_allocator::malloc(bytes);
        }
        static inline void free(void *obj)
        {
            ++_free_count();
            return tracking_allocator::free(obj);
        }
    };
    ...
    struct Copyable
    {
        ...
        void operator=(Copyable const &o)
        {
            copied = true;
            id = o.id;
        }
    ...
    {
        Moveable(int id)
            : moved(false), copied(false), id(id)
        {
        }
        Moveable(Moveable &&o) MOODYCAMEL_NOEXCEPT : moved(true), copied(o.copied), id(o.id) {}
        void operator=(Moveable &&o) MOODYCAMEL_NOEXCEPT
        {
            moved = true;
            copied = o.copied;
            id = o.id;
        ...
        void operator=(Moveable const &o) MOODYCAMEL_NOEXCEPT
        {
            moved = o.moved;
            copied = o.copied;
            id = o.id;
    ...
    struct ThrowingMovable
    {
        ...
        static std::atomic<int> &destroyCount()
        {
            static std::atomic<int> c;
            return c;
        }
    ...
    public:
        ...
        bool enqueue_bulk()
        {
            ...
            Traits4::reset();
            {
                ...
                // 要容纳 8 个元素，需要两个 block（BLOKC_SIZE 为 4）
                // 创建的 block pool 就有两个 block
        ...
        bool leftovers_destroyed()
        {
            ...
            Foo::reset();
            {
                ...
                Foo item;
                // construct item
                ASSERT_OR_FAIL(Foo::createCount() == 1);

                // construct Foo()
                // destruct Foo(): 创建的临时对象执行析构（和 std::move 无关。std::move 不执行析构）
                q.enqueue(Foo());
                ASSERT_OR_FAIL(Foo::createCount() == 2);
                ASSERT_OR_FAIL(Foo::destroyCount() == 1);

                // construct Foo()
                // destruct Foo()
                q.enqueue(Foo());
                ASSERT_OR_FAIL(Foo::createCount() == 3);
                ASSERT_OR_FAIL(Foo::destroyCount() == 2);

                // construct Foo()
                // destruct Foo()
                q.enqueue(Foo());
                ASSERT_OR_FAIL(Foo::createCount() == 4);
                ASSERT_OR_FAIL(Foo::destroyCount() == 3);

                // 元素出队时执行一次析构
                q.try_dequeue(item);
                ASSERT_OR_FAIL(Foo::createCount() == 4);
                ASSERT_OR_FAIL(Foo::destroyCount() == 4);
            }
            ASSERT_OR_FAIL(Foo::createCount() == 4);
            // 析构 q 时将队列中剩余的 2 个元素全部出队，执行这两个元素的析构
            // 析构 item
            ASSERT_OR_FAIL(Foo::destroyCount() == 7);
            ASSERT_OR_FAIL(Foo::destroyedInOrder());
        ...
        bool block_index_resized()
        {
            ...
            Foo::reset();
            {
                ...
                // new block pool times 1
                ASSERT_OR_FAIL(Traits::malloc_count() == 1);

                // new producer times 1

                /**
                 * @brief
                 * 总共需要申请的 blocks 数量为 1024 / 4 == 256。block pool 中的 blocks 数量为 8 / 4 == 2。所以还需要申请 254 个 block 才能够保存 1024 个元素。
                 *
                 * 初始的 blockIndex size 为 4。即第一次申请的 blockIndex 可以保存 4 个 block。所以 blockIndex 中还需要保存 256 - 4 = 252 个 block。也就是
                 * 说，当 blockIndex size 大于 252 时，就可以保存剩余所有的 blocks。
                 *
                 * 每次申请的 blockIndex size 为上一次申请的大小的两倍，并且会将上一次 blockIndex 中的 index 指针映射到本次创建的 blockIndex 中的 index 部分，
                 * 保证在最后一个 blockIndex 中就可以索引到所有 blockIndex 中保存的元素。也就是说，最后一个 blockIndex 的大小必须能够保存 252 个 block 才可以。
                 * 按照这样的逻辑，第一次申请的 blockIndex size 为 4，后续每次申请的 blockIndex size 依次为 8 16 32 64 128 256。当申请的 blockIndex 大小为
                 * 256 时，就可以保存剩余所有的 blocks。
                 */
                for (int i = 0; i != 1024; ++i)
                {
                    q.enqueue(Foo());
        ...
        struct TestListItem : corealgos::ListItem
        {
            ...
            {
                return ctorCount().load(std::memory_order_relaxed);
            }
            inline static size_t destructed()
            {
                return dtorCount().load(std::memory_order_relaxed);
            }

        private:
            inline static std::atomic<size_t> &ctorCount()
            {
                static std::atomic<size_t> count(0);
                return count;
            }
            inline static std::atomic<size_t> &dtorCount()
            {
                static std::atomic<size_t> count(0);
                return count;
            }
